...
3. Navigate to `https://x.com/hashtag/fsharp?src=hashtag_click&f=live` using `playwright_browser_navigate`.

4. Wait for the tweet feed to load using `playwright_browser_wait_for`.

5. **Use `playwright_browser_run_code` to extract tweet data via JavaScript** — do NOT use WebFetch, do NOT read page snapshots. Run JavaScript inside the tab to query the DOM:
   ```js
   async (page) => {
     return await page.evaluate(() => {
       return Array.from(document.querySelectorAll('[data-testid="tweet"], article[data-testid="tweet"]')).map(el => {
...
6. Scroll down to load more tweets using `playwright_browser_run_code`:
   ```js
   async (page) => {
   ...
   ```
   After each scroll batch, re-run the extraction JavaScript and check the dates of newly loaded tweets. Stop scrolling when you encounter tweets older than `dateFrom`.

7. Deduplicate by tweet URL (keep first occurrence).

8. Compute the output folder path:
...
9. Create the folder if it does not exist: `mkdir -p data/{year}/week-{NN}/`

10. Write results to `data/{year}/week-{NN}/twitter.json`:
...
12. Report: total tweets collected, date range covered, and the output file path.

## Important notes

- Open a **new tab**; do not reuse existing tabs.
...
- If the site fails to load, write an empty `items` array with an `"error"` field and still close the tab.
